...
internal sealed class FakeWebSocket : WebSocket
{
    ...
    private readonly Channel<byte[]> _micChannel = Channel.CreateUnbounded<byte[]>();
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        {
            int bytesRead = await microphoneInput.ReadAsync(buffer.AsMemory(0, buffer.Length), cancellationToken).ConfigureAwait(false);
            if (bytesRead == 0)
            {
    ...
    public override async Task<WebSocketReceiveResult> ReceiveAsync(ArraySegment<byte> buffer, CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            await foreach(var item in _micChannel.Reader.ReadAllAsync(cancellationToken))
            {
                if (item.Length > buffer.Count)
                {
                    throw new InvalidOperationException($"Provided receive buffer too small. Needed {item.Length}, had {buffer.Count}.");
                }
                Array.Copy(item, 0, buffer.Array, buffer.Offset, item.Length);
                return new WebSocketReceiveResult(item.Length, WebSocketMessageType.Text, endOfMessage: true);
            }
