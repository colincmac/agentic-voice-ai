...
{
    /// <summary>
    /// Process-wide default instance, used when callers (tests, examples) don't
    /// resolve <see cref="CallingTelemetry"/> from DI. Listeners attached to the
    /// shared <see cref="ActivitySource"/> / <see cref="Meter"/> names will still
    /// observe its instruments.
    /// </summary>
    public static CallingTelemetry Default { get; } = new();
