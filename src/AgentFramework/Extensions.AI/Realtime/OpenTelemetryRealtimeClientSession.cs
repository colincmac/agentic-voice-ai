...
#pragma warning restore IDE0060
    {
        ...
        string name = string.IsNullOrEmpty(sourceName) ? TelemetryHelpers.DefaultSourceName : sourceName!;
    ...
    public async Task SendAsync(RealtimeClientMessage message, CancellationToken cancellationToken = default)
    {
        if (EnableSensitiveData && _activitySource.HasListeners())
        {
            ...
            if (otelMessage is not null)
            {
                ...
                {
                    _ = inputActivity.AddTag(GenAI.AttributeGenAiInputMessages, SerializeMessage(otelMessage));
                }
            }
        }
    ...
    private static void AddOutputModalitiesTag(Activity? activity, HashSet<string>? outputModalities)
    {
        ...
        {
            _ = activity.AddTag(GenAI.Realtime.ReceivedModalities, $"[{string.Join(", ", outputModalities.Select(m => $"\"{m}\""))}]");
        }
    }
    ...
    private static void AddOutputMessagesTag(Activity? activity, List<RealtimeOtelMessage>? outputMessages)
    {
        ...
        {
            _ = activity.AddTag(GenAI.AttributeGenAiOutputMessages, SerializeMessages(outputMessages));
        }
    }
    ...
    private Activity? CreateAndConfigureActivity(RealtimeSessionOptions? options)
    {
        ...
        if (_activitySource.HasListeners())
        {
            ...
            activity = _activitySource.StartActivity(
                string.IsNullOrWhiteSpace(modelId) ? GenAI.OperationNameValues.RealtimeName : $"{GenAI.OperationNameValues.RealtimeName} {modelId}",
                ActivityKind.Client);

            if (activity is { IsAllDataRequested: true })
            {
                _ = activity
                    .AddTag(GenAI.AttributeGenAiOperationName, GenAI.OperationNameValues.ChatName)
                    .AddTag(GenAI.AttributeGenAiRequestModel, modelId)
                    .AddTag(GenAI.AttributeGenAiProviderName, _providerName);
                ...
                if (options is not null)
                {
                    ...
                    {
                        _ = activity.AddTag(GenAI.AttributeGenAiRequestMaxTokens, maxTokens);
                    }

                    // Realtime-specific attributes
                    _ = activity.AddTag(GenAI.Realtime.SessionKind, options.SessionKind.ToString());

                    if (!string.IsNullOrEmpty(options.Voice))
                    {
                        _ = activity.AddTag(GenAI.Realtime.Voice, options.Voice);
                    }

                    if (options.OutputModalities is { Count: > 0 } modalities)
                    {
                        _ = activity.AddTag(GenAI.Realtime.OutputModalities, $"[{string.Join(", ", modalities.Select(m => $"\"{m}\""))}]");
                    }
                    ...
                    if (EnableSensitiveData)
                    {
                        ...
                        {
                            _ = activity.AddTag(
                                GenAI.AttributeGenAiSystemInstructions,
                    ...
                    {
                        _ = activity.AddTag(GenAI.AttributeGenAiRequestToolChoice, toolChoice);
                    }
                }
            }
        }
        ...
        Stopwatch? stopwatch)
    {
        ...
        if (_tokenUsageHistogram.Enabled && response?.Usage is { } usage)
        {
            ...
            {
                TagList tags = default;
                tags.Add(GenAI.AttributeGenAiTokenType, TokenTypeValues.Input);
                AddMetricTags(ref tags, requestModelId, responseModelId: null);
            ...
            {
                TagList tags = default;
                tags.Add(GenAI.AttributeGenAiTokenType, TokenTypeValues.Output);
                AddMetricTags(ref tags, requestModelId, responseModelId: null);
            ...
            {
                TagList tags = default;
                tags.Add(GenAI.AttributeGenAiTokenType, TokenTypeValues.InputAudio);
                AddMetricTags(ref tags, requestModelId, responseModelId: null);
            ...
            {
                TagList tags = default;
                tags.Add(GenAI.AttributeGenAiTokenType, TokenTypeValues.InputText);
                AddMetricTags(ref tags, requestModelId, responseModelId: null);
            ...
            {
                TagList tags = default;
                tags.Add(GenAI.AttributeGenAiTokenType, TokenTypeValues.OutputAudio);
                AddMetricTags(ref tags, requestModelId, responseModelId: null);
            ...
            {
                TagList tags = default;
                tags.Add(GenAI.AttributeGenAiTokenType, TokenTypeValues.OutputText);
                AddMetricTags(ref tags, requestModelId, responseModelId: null);
        ...
        if (response is not null && activity is not null)
        {
            ...
            {
                _ = activity.AddTag(GenAI.AttributeGenAiResponseId, response.ResponseId);
            }

            if (!string.IsNullOrWhiteSpace(response.Status))
            {
                _ = activity.AddTag(GenAI.AttributeGenAiResponseFinishReasons, $"[\"{response.Status}\"]");
            }
            ...
            if (response.Usage is { } responseUsage)
            {
                ...
                {
                    _ = activity.AddTag(GenAI.Usage.InputTokens, (int)inputTokens);
                }

                if (responseUsage.OutputTokenCount is long outputTokens)
                {
                    _ = activity.AddTag(GenAI.Usage.OutputTokens, (int)outputTokens);
                }

                if (responseUsage.CachedInputTokenCount is long cachedInputTokens)
                {
                    _ = activity.AddTag(GenAI.Usage.CacheReadInputTokens, (int)cachedInputTokens);
                }

                if (responseUsage.InputAudioTokenCount is long inputAudioTokens)
                {
                    _ = activity.AddTag(GenAI.Usage.InputAudioTokens, (int)inputAudioTokens);
                }

                if (responseUsage.InputTextTokenCount is long inputTextTokens)
                {
                    _ = activity.AddTag(GenAI.Usage.InputTextTokens, (int)inputTextTokens);
                }

                if (responseUsage.OutputAudioTokenCount is long outputAudioTokens)
                {
                    _ = activity.AddTag(GenAI.Usage.OutputAudioTokens, (int)outputAudioTokens);
                }

                if (responseUsage.OutputTextTokenCount is long outputTextTokens)
                {
                    _ = activity.AddTag(GenAI.Usage.OutputTextTokens, (int)outputTextTokens);
                }
            }
            ...
            {
                _ = activity.AddTag(Error.AttributeErrorType, responseError.ErrorCode ?? "RealtimeError");
    ...
    {
        tags.Add(GenAI.AttributeGenAiOperationName, GenAI.OperationNameValues.ChatName);
        ...
        {
            tags.Add(Server.Address, endpointAddress);
