...
{
    public static IResourceBuilder<AIModel> AddAIModel(this IDistributedApplicationBuilder builder, string name, IResourceBuilder<ParameterResource> modelDeploymentName)
    {
        var model = new AIModel(name, modelDeploymentName.Resource);
    ...
    public static IResourceBuilder<AIModel> RunAsAzureOpenAI(this IResourceBuilder<AIModel> builder, Action<IResourceBuilder<AzureOpenAIResource>>? configure)
    {
        if (builder.ApplicationBuilder.ExecutionContext.IsRunMode)
        {
            return builder.AsAzureOpenAI(configure);
        }

        return builder;
    }

    public static IResourceBuilder<AIModel> PublishAsAzureOpenAI(this IResourceBuilder<AIModel> builder, Action<IResourceBuilder<AzureOpenAIResource>>? configure)
    {
        if (builder.ApplicationBuilder.ExecutionContext.IsPublishMode)
        {
            return builder.AsAzureOpenAI(configure);
    ...
    public static IResourceBuilder<AIModel> AsAzureOpenAI(this IResourceBuilder<AIModel> builder, Action<IResourceBuilder<AzureOpenAIResource>>? configure, IResourceBuilder<ParameterResource>? apiKey = null)
    {
        builder.Reset();
        ...
        builder.Resource.ConnectionString = ReferenceExpression.Create($"{openAIModel.Resource.ConnectionStringExpression};Model={builder.Resource.ModelParameter.GetValueAsync(CancellationToken.None).Result}");
...
public class AIModel(string name, ParameterResource model) : Resource(name), IResourceWithConnectionString
{
    internal string? Provider { get; set; }
    public ParameterResource ModelParameter { get; } = model;
