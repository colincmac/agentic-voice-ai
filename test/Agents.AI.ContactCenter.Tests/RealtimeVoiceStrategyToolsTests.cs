...
public class RealtimeVoiceStrategyToolsTests
{
    ...
    public async Task StartAsync_pushes_guard_wrapped_tools_for_initial_step()
    {
        ...
        await using var strategy = new RealtimeVoiceStrategy(backend, IvrWorkflowSession.Create(workflow, new ServiceCollection().AddLogging().BuildServiceProvider()), TestTelemetry.LoggerFactory, TestTelemetry.Calling);
